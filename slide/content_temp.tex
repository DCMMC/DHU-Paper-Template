...
%	\quad
	\vskip .2em
	\Large \textcolor[rgb]{0.1098,0.2706,0.5294}{{\fontspec{Noto Sans CJK SC} 工作汇报} {\fontspec{Comic Sans MS} 2020-04-06}}

	\vskip 1.2em
	\large \textcolor[rgb]{0.1098,0.2706,0.5294}{{\fontspec{Noto Sans CJK SC} 肖文韬}
...
%	\footnotesize\textcolor{gray}{Based on Flip Beamer Theme by Flip Tanedo @ Cornell University}\normalsize\\
	\textcolor{normal text.fg!50!Comment}{\textit{Donghua University}, \today}
\end{center}
\end{frame}
}
...
\begin{frame}[c]{Beamer Theme Flip}{A work in progress}
...
\begin{frame}[c]{Example of the \texttt{pause} command}
	In this slide \pause
...
\begin{frame}[fragile]{Examples of \texttt{minted} for syntax highlighting}
	\begin{overprint}
		\onslide<1>
		\begin{minted}[texcomments,linenos,tabsize=2,highlightlines={17-18},fontsize=\tiny,style=friendly,bgcolor=friendlybg]{python}
...
\begin{frame}
	\only<1>{\frametitle{Example of Algorithm}}
	...
	\begin{minipage}[b]{0.60\linewidth}
		\centering
		\only<1-5>{\setcounter{algorithm}{0}}
		\begin{algorithm}[H]
			\scriptsize
			\begin{algorithmic}
...
\begin{frame}[c]{Color styles}
	These are some useful pre-defined color styles.
		...
		\end{columns}
		\vspace{1em}
		Some colors like \textcolor{FlipGreen}{FlipGreen} and \textcolor{FlipSand}{FlipSand} will automatically change tint when when you define light or dark bgrounds. \Comment{This makes it easier to swap between light/dark backgrounds by just modifying one option and recompiling.}
...
\section{Samples of Tikz Diagrams}
...
\section{Tables and Math Equations}
